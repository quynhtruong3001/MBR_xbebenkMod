	...
	If $g_iCommandStop <> 3 And $g_iCommandStop <> 0 Then
		...
		While True
			...
			If AttackMain($g_bSkipDT) Then
				$successcount += 1
				Setlog("[" & $successcount & "/5] 1st Attack Loop Success", $COLOR_SUCCESS)
				RequestCC()
				If $successcount >= 5 Then ExitLoop
			Else
				If $g_bForceSwitch Then ExitLoop ;exit here
				$loopcount += 1
				If $loopcount > 50 Then
					Setlog("1st Attack Loop, Already Try 50 times... Exit", $COLOR_ERROR)
					ExitLoop
				Else
	...
	If ProfileSwitchAccountEnabled() Then
		...
		If $g_iCommandStop <> 0 And $g_iCommandStop <> 3 And Not $g_bChkAttackOnce Then
			...
			While True
				...
				If AttackMain($g_bSkipDT) Then
					$successcount += 1
					Setlog("[" & $successcount & "/5] 2nd Attack Loop Success", $COLOR_SUCCESS)
					$b_SuccessAttack = True
					If $successcount >= 5 Then ExitLoop
				Else
					$loopcount += 1
					If $loopcount > 50 Then
						Setlog("2nd Attack Loop, Already Try 50 times... Exit", $COLOR_ERROR)
						ExitLoop
					Else
...
Func CommonRoutine($RoutineType = Default)
	...
	Switch $RoutineType
		Case "FirstCheck"
			Local $aRndFuncList = ['Collect', 'CollectLootCart', 'TreasuryCollect', 'CleanYard', 'CollectCookie', 'UseFreeMagic', 'BoostSuperTroop', 'PetHouse', 'ForgeClanCapitalGold', 'CollectCCGold', 'AutoUpgradeCC', 'BlackSmith', 'PetHouse', 'Laboratory', 'SellMagicItem', 'UpgradeWall']
		...
		Case "Switch"
			...
			Local $aRndFuncList = ['RequestCC', 'DailyChallenge', 'CollectAchievements', 'CollectFreeMagicItems', 'BoostSuperTroop', 'BuilderBase', 'Laboratory', 'UpgradeHeroes', 'UpgradeBuilding', 'AutoUpgrade', 'UpgradeWall', 'UpgradeLow', 'RequestCC']
			SetLog($RoutineType & " Func List:", $COLOR_SUCCESS)
			For $i In $aRndFuncList
				SetLog(" --> " & $i, $COLOR_NAVY)
			Next
		...
		Case "Idle"
			Local $aRndFuncList = ['Collect', 'CollectLootCart', 'TreasuryCollect', 'CleanYard', 'CollectCookie', 'UseFreeMagic', 'BoostSuperTroop', 'RequestCC', 'DailyChallenge', 'CollectAchievements', 'CollectFreeMagicItems', 'Laboratory', 'UpgradeHeroes', 'UpgradeBuilding', 'AutoUpgrade', 'UpgradeWall', 'UpgradeLow', 'BuilderBase']
			SetLog($RoutineType & " Func List:", $COLOR_SUCCESS)
			For $i In $aRndFuncList
				SetLog(" --> " & $i, $COLOR_NAVY)
			Next
		...
		Case "NoClanGamesEvent"
			Local $aRndFuncList = ['RequestCC', 'DailyChallenge', 'CollectAchievements', 'CollectFreeMagicItems', 'BoostSuperTroop', 'PetHouse', 'Laboratory', 'CollectCCGold', 'UpgradeHeroes', 'UpgradeBuilding', 'AutoUpgrade', 'UpgradeWall', 'UpgradeLow']
			SetLog($RoutineType & " Func List:", $COLOR_SUCCESS)
			For $i In $aRndFuncList
				SetLog(" --> " & $i, $COLOR_NAVY)
			Next
