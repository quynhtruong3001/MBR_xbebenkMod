...
Func FirstCheckRoutine()
	...
	If ProfileSwitchAccountEnabled() Then
		...
		If BotCommand() Then btnStop()
		If Not $g_bRunState Then Return
		Local $b_SecondAttackSuccess = False ; Track success of the second attack
		If $g_iCommandStop <> 0 And $g_iCommandStop <> 3 Then
			Setlog("Let's attack Again (Second Attack)!", $COLOR_INFO)
			...
			While True
				...
				If AttackMain($g_bSkipDT) Then
					...
					$b_SecondAttackSuccess = True ; Mark second attack as successful
					ExitLoop
				Else
					$loopcount += 1
					If $loopcount > 10 Then
		...
		Local $b_ThirdAttackSuccess = False ; Track success of the third attack

		; **** NEW: Third Attack Loop ****
		If $b_SecondAttackSuccess Then ; Only attempt third if second was successful
			checkMainScreen() ; Ensure we are back on the main screen after second attack
			SetLog("Check Third Attack", $COLOR_ACTION)
			FillArmyCamp()
			If BotCommand() Then btnStop()
			If Not $g_bRunState Then Return
			If $g_iCommandStop <> 0 And $g_iCommandStop <> 3 Then
				Setlog("Let's attack Again (Third Attack)!", $COLOR_INFO)
				$g_bRestart = False ;reset
				Local $loopcount3 = 1
				While True
					$g_bRestart = False
					If Not $g_bRunState Then Return
					If AttackMain($g_bSkipDT) Then
						Setlog("[" & $loopcount3 & "] 3rd Attack Loop Success", $COLOR_SUCCESS)
						$b_ThirdAttackSuccess = True ; Mark third attack as successful
						ExitLoop
					Else
						$loopcount3 += 1
						If $loopcount3 > 10 Then
							Setlog("3rd Attack Loop, Already Try 10 times... Exit", $COLOR_ERROR)
							ExitLoop
						Else
							Setlog("[" & $loopcount3 & "] 3rd Attack Loop, Failed", $COLOR_INFO)
							If $g_bForceSwitch Then ExitLoop
						EndIf
						If Not $g_bRunState Then Return
					EndIf
		...
		Local $b_FourthAttackSuccess = False ; Track success of the fourth attack
		; **** NEW: Fourth Attack Loop ****
		If $b_ThirdAttackSuccess Then ; Only attempt fourth if third was successful
			checkMainScreen() ; Ensure we are back on the main screen after third attack
			SetLog("Check Fourth Attack", $COLOR_ACTION)
			FillArmyCamp()
			If BotCommand() Then btnStop()
			If Not $g_bRunState Then Return
			If $g_iCommandStop <> 0 And $g_iCommandStop <> 3 Then
				Setlog("Let's attack Again (Fourth Attack)!", $COLOR_INFO)
				$g_bRestart = False ;reset
				Local $loopcount4 = 1
				While True
					$g_bRestart = False
					If Not $g_bRunState Then Return
					If AttackMain($g_bSkipDT) Then
						Setlog("[" & $loopcount4 & "] 4th Attack Loop Success", $COLOR_SUCCESS)
						$b_FourthAttackSuccess = True ; Mark fourth attack as successful
						ExitLoop
					Else
						$loopcount4 += 1
						If $loopcount4 > 10 Then
							Setlog("4th Attack Loop, Already Try 10 times... Exit", $COLOR_ERROR)
							ExitLoop
						Else
							Setlog("[" & $loopcount4 & "] 4th Attack Loop, Failed", $COLOR_INFO)
							If $g_bForceSwitch Then ExitLoop
						EndIf
						If Not $g_bRunState Then Return
					EndIf
		...
		Local $b_FifthAttackSuccess = False ; Track success of the fourth attack
				; **** NEW: Fifth Attack Loop ****
		If $b_FourthAttackSuccess Then ; Only attempt fourth if third was successful
			checkMainScreen() ; Ensure we are back on the main screen after third attack
			SetLog("Check Fifth Attack", $COLOR_ACTION)
			FillArmyCamp()
			If BotCommand() Then btnStop()
			If Not $g_bRunState Then Return
			If $g_iCommandStop <> 0 And $g_iCommandStop <> 3 Then
				Setlog("Let's attack Again (Fifth Attack)!", $COLOR_INFO)
				$g_bRestart = False ;reset
				Local $loopcount5 = 1
				While True
					$g_bRestart = False
					If Not $g_bRunState Then Return
					If AttackMain($g_bSkipDT) Then
						Setlog("[" & $loopcount5 & "] 5th Attack Loop Success", $COLOR_SUCCESS)
						$b_FifthAttackSuccess = True ; Mark fifth attack as successful
						ExitLoop
					Else
						$loopcount5 += 1
						If $loopcount5 > 10 Then
							Setlog("5th Attack Loop, Already Try 10 times... Exit", $COLOR_ERROR)
							ExitLoop
						Else
							Setlog("[" & $loopcount5 & "] 5th Attack Loop, Failed", $COLOR_INFO)
							If $g_bForceSwitch Then ExitLoop
						EndIf
						If Not $g_bRunState Then Return
					EndIf
		...
		Local $b_SixthAttackSuccess = False ; Track success of the fourth attack
						; **** NEW: Sixth Attack Loop ****
		If $b_FifthAttackSuccess Then ; Only attempt fourth if third was successful
			checkMainScreen() ; Ensure we are back on the main screen after third attack
			SetLog("Check Sixth Attack", $COLOR_ACTION)
			FillArmyCamp()
			If BotCommand() Then btnStop()
			If Not $g_bRunState Then Return
			If $g_iCommandStop <> 0 And $g_iCommandStop <> 3 Then
				Setlog("Let's attack Again (Sixth Attack)!", $COLOR_INFO)
				$g_bRestart = False ;reset
				Local $loopcount6 = 1
				While True
					$g_bRestart = False
					If Not $g_bRunState Then Return
					If AttackMain($g_bSkipDT) Then
						Setlog("[" & $loopcount6 & "] 6th Attack Loop Success", $COLOR_SUCCESS)
						$b_SixthAttackSuccess = True ; Mark fifth attack as successful
						ExitLoop
					Else
						$loopcount6 += 1
						If $loopcount6 > 10 Then
							Setlog("6th Attack Loop, Already Try 10 times... Exit", $COLOR_ERROR)
							ExitLoop
						Else
							Setlog("[" & $loopcount6 & "] 6th Attack Loop, Failed", $COLOR_INFO)
							If $g_bForceSwitch Then ExitLoop
						EndIf
						If Not $g_bRunState Then Return
					EndIf
		...
		Local $b_SeventhAttackSuccess = False ; Track success of the fourth attack
								; **** NEW: Seventh Attack Loop ****
		If $b_SixthAttackSuccess Then ; Only attempt fourth if third was successful
			checkMainScreen() ; Ensure we are back on the main screen after third attack
			SetLog("Check Seventh Attack", $COLOR_ACTION)
			FillArmyCamp()
			If BotCommand() Then btnStop()
			If Not $g_bRunState Then Return
			If $g_iCommandStop <> 0 And $g_iCommandStop <> 3 Then
				Setlog("Let's attack Again (Seventh Attack)!", $COLOR_INFO)
				$g_bRestart = False ;reset
				Local $loopcount7 = 1
				While True
					$g_bRestart = False
					If Not $g_bRunState Then Return
					If AttackMain($g_bSkipDT) Then
						Setlog("[" & $loopcount7 & "] 7th Attack Loop Success", $COLOR_SUCCESS)
						$b_SeventhAttackSuccess = True ; Mark seventh attack as successful
						ExitLoop
					Else
						$loopcount7 += 1
						If $loopcount7 > 10 Then
							Setlog("7th Attack Loop, Already Try 10 times... Exit", $COLOR_ERROR)
							ExitLoop
						Else
							Setlog("[" & $loopcount7 & "] 7th Attack Loop, Failed", $COLOR_INFO)
							If $g_bForceSwitch Then ExitLoop
						EndIf
						If Not $g_bRunState Then Return
					EndIf
		...
		Local $b_EighthAttackSuccess = False ; Track success of the fourth attack
										; **** NEW: 8th Attack Loop ****
		If $b_SeventhAttackSuccess Then ; Only attempt fourth if third was successful
			checkMainScreen() ; Ensure we are back on the main screen after third attack
			SetLog("Check Eighth Attack", $COLOR_ACTION)
			FillArmyCamp()
			If BotCommand() Then btnStop()
			If Not $g_bRunState Then Return
			If $g_iCommandStop <> 0 And $g_iCommandStop <> 3 Then
				Setlog("Let's attack Again (Eighth Attack)!", $COLOR_INFO)
				$g_bRestart = False ;reset
				Local $loopcount8 = 1
				While True
					$g_bRestart = False
					If Not $g_bRunState Then Return
					If AttackMain($g_bSkipDT) Then
						Setlog("[" & $loopcount8 & "] 8th Attack Loop Success", $COLOR_SUCCESS)
						$b_EighthAttackSuccess = True ; Mark seventh attack as successful
						ExitLoop
					Else
						$loopcount8 += 1
						If $loopcount8 > 10 Then
							Setlog("8th Attack Loop, Already Try 10 times... Exit", $COLOR_ERROR)
							ExitLoop
						Else
							Setlog("[" & $loopcount8 & "] 8th Attack Loop, Failed", $COLOR_INFO)
							If $g_bForceSwitch Then ExitLoop
						EndIf
						If Not $g_bRunState Then Return
					EndIf
		...
		Local $b_NinethAttackSuccess = False ; Track success of the fourth attack
												; **** NEW: 9th Attack Loop ****
		If $b_EighthAttackSuccess Then ; Only attempt fourth if third was successful
			checkMainScreen() ; Ensure we are back on the main screen after third attack
			SetLog("Check Nineth Attack", $COLOR_ACTION)
			FillArmyCamp()
			If BotCommand() Then btnStop()
			If Not $g_bRunState Then Return
			If $g_iCommandStop <> 0 And $g_iCommandStop <> 3 Then
				Setlog("Let's attack Again (Nineth Attack)!", $COLOR_INFO)
				$g_bRestart = False ;reset
				Local $loopcount9 = 1
				While True
					$g_bRestart = False
					If Not $g_bRunState Then Return
					If AttackMain($g_bSkipDT) Then
						Setlog("[" & $loopcount9 & "] 9th Attack Loop Success", $COLOR_SUCCESS)
						$b_NinethAttackSuccess = True ; Mark seventh attack as successful
						ExitLoop
					Else
						$loopcount9 += 1
						If $loopcount9 > 10 Then
							Setlog("9th Attack Loop, Already Try 10 times... Exit", $COLOR_ERROR)
							ExitLoop
						Else
							Setlog("[" & $loopcount9 & "] 9th Attack Loop, Failed", $COLOR_INFO)
							If $g_bForceSwitch Then ExitLoop
						EndIf
						If Not $g_bRunState Then Return
					EndIf
		...
		Local $b_TenthAttackSuccess = False ; Track success of the fourth attack
														; **** NEW: 10th Attack Loop ****
		If $b_NinethAttackSuccess Then ; Only attempt fourth if third was successful
			checkMainScreen() ; Ensure we are back on the main screen after third attack
			SetLog("Check Tenth Attack", $COLOR_ACTION)
			FillArmyCamp()
			If BotCommand() Then btnStop()
			If Not $g_bRunState Then Return
			If $g_iCommandStop <> 0 And $g_iCommandStop <> 3 Then
				Setlog("Let's attack Again (Tenth Attack)!", $COLOR_INFO)
				$g_bRestart = False ;reset
				Local $loopcount10 = 1
				While True
					$g_bRestart = False
					If Not $g_bRunState Then Return
					If AttackMain($g_bSkipDT) Then
						Setlog("[" & $loopcount10 & "] 10th Attack Loop Success", $COLOR_SUCCESS)
						$b_TenthAttackSuccess = True ; Mark seventh attack as successful
						ExitLoop
					Else
						$loopcount10 += 1
						If $loopcount10 > 10 Then
							Setlog("10th Attack Loop, Already Try 10 times... Exit", $COLOR_ERROR)
							ExitLoop
						Else
							Setlog("[" & $loopcount10 & "] 10th Attack Loop, Failed", $COLOR_INFO)
							If $g_bForceSwitch Then ExitLoop
						EndIf
						If Not $g_bRunState Then Return
					EndIf
		...
		; Ensure we are back on the main screen before switching
		checkMainScreen()
		checkSwitchAcc() ;switch to next account
	EndIf
